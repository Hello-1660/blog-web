...
import { getArticleList, getUserArticlesById } from '@/apis/user'
...
const handleAddArticle = () => { router.push('/article/edit') }
const handleUpdateArticle = (id: number) => { router.push({ name: 'articleEdit', params: { id } }) }
const handleDeleteArticle = (id: number) => { deleteTargetId.value = id; showDeleteModal.value = true }
...
const handleMouseEnter = (el: MouseEvent) => {
  (el.currentTarget as HTMLElement).classList.add('article-item-show-function')
}
const handleMouseLeave = (el: MouseEvent) => {
  (el.currentTarget as HTMLElement).classList.remove('article-item-show-function')
}

onMounted(async () => {
  const data = porp.userId
    ? await getUserArticlesById(porp.userId)
...
<template>
  <div class="composition-container">
    <div v-if="isOwner" class="article-add" @click="handleAddArticle">
    ...
    />
      <div class="article-icon"><img :src="article.icon" alt=""></div>
...
.article-add>.icon { width: 40px; height: 40px; fill: var(--font-color) }
.article-add:hover { transform: scale(1.05) }
.article-item {
  position: relative;
  display: flex;
...
.article-item:hover { transform: scale(1.05) }
.article-item-show-function>.function-list { display: flex }
.function-list { display: none }
.article-icon { width: 100% !important; height: 170px; padding: 5px }
.article-icon > img { width: 100%; height: 100% }
.article-show { display: flex; flex-direction: column; width: 100%; height: 100%; padding: 10px }
.article-body { flex: 1 }
.article-detail { display: flex; justify-content: start }
.article-title { font-size: 40px; font-weight: 400; margin-bottom: 10px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis }
.article-status { font-size: 15px; font-weight: 200; margin-left: 10px; padding-top: 10px }
.icon { width: 20px; height: 20px }
.article-content { font-size: 15px; font-weight: 200; overflow: hidden; white-space: nowrap; text-overflow: ellipsis }
.article-footer { width: 250px; height: 20px; margin-top: 10px }
.article-time { --time-font-height: 20px; width: 80%; height: var(--time-font-height); line-height: var(--time-font-height); text-align: left; color: #8e8e8e }
.delete-confirm { min-width: 280px }
.delete-confirm p { font-size: 14px; margin-bottom: 20px; line-height: 1.6 }
.delete-confirm-footer { display: flex; justify-content: flex-end; gap: 8px }
.btn-cancel { padding: 6px 16px; border: none; border-radius: 4px; background-color: var(--hover-bgc); color: var(--font-color); font-size: 13px; cursor: pointer }
.btn-confirm { padding: 6px 16px; border: none; border-radius: 4px; background-color: #e74c3c; color: #fff; font-size: 13px; cursor: pointer }
